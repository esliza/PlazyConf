...
class UbicationFragment : Fragment(),OnMapReadyCallback,GoogleMap.OnMarkerClickListener{
    ...
    override fun onMapReady(googleMap: GoogleMap?) {
        ...
        val centermarker = LatLng(ubication.latitude, ubication.longitude)
        val marketOptions = MarkerOptions()
        marketOptions.position(centermarker)
        marketOptions.title("Platzy Conf 2019")

        val bitmapDraw = context?.applicationContext?.let {
            ContextCompat.getDrawable(it,R.drawable.logo_platzi)
        } as BitmapDrawable
        val smallmarket = Bitmap.createScaledBitmap(bitmapDraw.bitmap,150, 150, false)
        marketOptions.icon(BitmapDescriptorFactory.fromBitmap(smallmarket))
    ...
    override fun onMarkerClick(p0: Marker?): Boolean {
        findNavController().navigate(R.id.mapDetailFragment)
        return true
    }
}